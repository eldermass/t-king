...
type Tone = '' | 'is-strong' | 'is-good' | 'is-neutral' | 'is-weak' | 'is-poor'
...
const scoreLoading = computed(() => loading.value || refreshing.value)
const loadError = ref('')
...
const scoreTone = (value: number | null, risk = false): Tone => {
  if (value === null || !Number.isFinite(value)) return ''
  const adjusted = risk ? 100 - value : value
  return adjusted >= 80 ? 'is-strong' : adjusted >= 60 ? 'is-good' : adjusted >= 40 ? 'is-neutral' : adjusted >= 20 ? 'is-weak' : 'is-poor'
}
const historyTone = (value: number | null): Tone => {
  if (value === null || !Number.isFinite(value)) return ''
  return value >= 60 ? 'is-strong' : value <= 40 ? 'is-weak' : 'is-neutral'
}
...
const detailTone = (field: string, high: number, low: number, higherIsGood = true): Tone => {
  const current = marketValue(field)
  if (current === null || !Number.isFinite(current)) return ''
  const range = high - low
  if (range <= 0) return ''
  const normalized = higherIsGood ? (current - low) / range : (high - current) / range
  return normalized > 1 ? 'is-strong' : normalized > 2 / 3 ? 'is-good' : normalized >= 1 / 3 ? 'is-neutral' : normalized >= 0 ? 'is-weak' : 'is-poor'
}
const metricDetails = (key: MetricKey): DetailItem[] => {
  const value = (field: string, label: string, formatter: (item: number | null) => string = integerText, tone: Tone = ''): DetailItem => ({ label, value: formatter(marketValue(field)), tone })
  const statusValue = (field: string, label: string, high: number, low: number, formatter: (item: number | null) => string = integerText, higherIsGood = true) => value(field, label, formatter, detailTone(field, high, low, higherIsGood))
  const percent = (field: string, label: string, high: number, low: number, higherIsGood = true) => statusValue(field, label, high, low, percentText, higherIsGood)
  if (key === 'profitScore') return [percent('yesterdayLimitUpReturn', '涨停平均收益', .03, 0), percent('yesterdayLimitUpMedianReturn', '涨停中位数', .03, 0), percent('yesterdayLimitUpRiseRatio', '涨停上涨比例', .6, .3), percent('yesterdayLadderReturn', '连板收益', .03, 0), percent('leaderReturn', '最高板收益', .03, 0)]
  if (key === 'speculationScore') return [statusValue('limitUp', '涨停家数', 80, 30), statusValue('maxBoard', '最高连板', 6, 3), statusValue('board1', '一板', 40, 15), statusValue('board2', '二板', 15, 5), statusValue('board3', '三板', 10, 3), statusValue('board4Plus', '四板+', 8, 3), statusValue('board5', '五板', 5, 2), statusValue('board6', '六板', 3, 1), statusValue('board7Plus', '七板+', 2, 0)]
  if (key === 'breadthScore') return [value('totalStocks', '股票总数'), statusValue('advancers', '上涨家数', 4000, 1000), statusValue('decliners', '下跌家数', 4000, 1000, integerText, false), value('unchanged', '平盘家数')]
  if (key === 'limitScore') return [statusValue('limitUp', '涨停家数', 100, 30), statusValue('limitDown', '跌停家数', 20, 10, integerText, false), statusValue('brokenBoard', '炸板家数', 30, 10, integerText, false), percent('brokenBoardRate', '炸板率', .3, .15, false)]
  if (key === 'liquidityScore') return [statusValue('totalAmount', '两市成交额', 1500000000000, 800000000000, amountText), percent('amountChange', '成交额变化', .05, -.05)]
  return [statusValue('limitDown', '跌停家数', 20, 10, integerText, false), percent('brokenBoardRate', '炸板率', .3, .15, false), percent('leaderReturn', '高位股收益', .03, 0)]
}
